...
function setup() {
  ...
  blob = new Blob(
    ...
    random(-worldSize.height / 2, worldSize.height / 2),
    16,
    [
...
function drawGrid() {
  ...
  for (
    let x = leftBound - (leftBound % gridSpacing);
    x < rightBound;
    x += gridSpacing
  ) {
  ...
  for (
    let y = topBound - (topBound % gridSpacing);
    y < bottomBound;
    y += gridSpacing
  ) {
...
function displayScoreboard() {
  ...
  for (var i = 0; i < Math.min(5, sortedBlobs.length); i++) {
    ...
    var scoreText =
      ...
      sortedBlobs[i].id +
      ": " +
      sortedBlobs[i].r.toFixed(2); // Display blob ID and size
